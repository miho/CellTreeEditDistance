...
public class DefaultClusteringAlgorithm implements ClusteringAlgorithm
{
    ...
    @Override
    public Cluster performClustering(float[][] distances,
    ...
    @Override
    public List<Cluster> performFlatClustering(float[][] distances,
                                               String[] clusterNames, LinkageStrategy linkageStrategy, Float threshold)
    {
    ...
    private void checkArguments(float[][] distances, String[] clusterNames,
    ...
    @Override
    public Cluster performWeightedClustering(float[][] distances, String[] clusterNames,
                                             float[] weights, LinkageStrategy linkageStrategy)
    {
    ...
    private DistanceMap createLinkages(float[][] distances,
    ...
    private List<Cluster> createClusters(String[] clusterNames, float[] weights)
    {
        List<Cluster> clusters = new ArrayList<Cluster>();
        for (int i = 0; i < weights.length; i++)
        {
            Cluster cluster = new Cluster(clusterNames[i]);
            cluster.setDistance(new Distance(0.0f, weights[i]));
            clusters.add(cluster);
        }
        return clusters;
    }
