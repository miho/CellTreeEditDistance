...
public class ClusterComponent implements Paintable {
	...
	public ClusterComponent (Cluster cluster, boolean printName, VCoord initPoint, List<ClusterColorRegex> clusterColorRegexes) {
		this.printName = printName;
		this.cluster = cluster;
		this.initPoint = initPoint;
    ...
    public void paint(Graphics2D g, int xDisplayOffset, int yDisplayOffset, double xDisplayFactor, double yDisplayFactor, boolean decorated) {
        ...
        if (cluster.isLeaf()) {
            if (clusterColorRegexes != null && !clusterColorRegexes.isEmpty()) {
                for (ClusterColorRegex clusterColorRegex : clusterColorRegexes) {
                    if (Pattern.matches(clusterColorRegex.regex.toString(), cluster.getName())) {
                        g.setPaint(clusterColorRegex.color);
                        break;
                    }
                }
            }
