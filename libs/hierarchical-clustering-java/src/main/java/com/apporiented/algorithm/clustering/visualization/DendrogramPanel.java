...
import com.apporiented.algorithm.clustering.WardLinkageStrategy;
...
public class DendrogramPanel extends JPanel {
    ...
    private List<ClusterColorRegex> clusterColorRegexes;
    ...
    public List<ClusterColorRegex> getClusterColorRegexes() {
        return clusterColorRegexes;
    }

    public void setClusterColorRegexes(List<ClusterColorRegex> clusterColorRegexes) {
        this.clusterColorRegexes = clusterColorRegexes;
    }
    ...
    private ClusterComponent createComponent(Cluster cluster, VCoord initCoord, double clusterHeight) {
        ...
        if (cluster != null) {
            comp = new ClusterComponent(cluster, cluster.isLeaf(), initCoord, clusterColorRegexes);
    ...
    public static void main(String[] args) {
        ...
        List<ClusterColorRegex> clusterColorRegexes = Arrays.asList(
                new ClusterColorRegex(Pattern.compile("^O1$"), Color.BLACK),
                new ClusterColorRegex(Pattern.compile("^O2$"), Color.RED),
                new ClusterColorRegex(Pattern.compile("^O3$"), Color.BLUE),
                new ClusterColorRegex(Pattern.compile("^O4$"), Color.GREEN),
                new ClusterColorRegex(Pattern.compile("^O5$"), Color.ORANGE),
                new ClusterColorRegex(Pattern.compile("^O6$"), Color.GRAY)
        );
    ...
    private static Cluster createSampleCluster() {
        float[][] distances = new float[][]{{0, 1, 9, 7, 11, 14}, {1, 0, 4, 3, 8, 10}, {9, 4, 0, 9, 2, 8},
                {7, 3, 9, 0, 6, 13}, {11, 8, 2, 6, 0, 10}, {14, 10, 8, 13, 10, 0}};
        String[] names = new String[]{"O1", "O2", "O3", "O4", "O5", "O6"};
        ClusteringAlgorithm alg = new DefaultClusteringAlgorithm();
        Cluster cluster = alg.performClustering(distances, names, new WardLinkageStrategy());
        cluster.toConsole(0);
        return cluster;
    }
