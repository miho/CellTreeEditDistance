...
//        }
        ...
        File metadataPath = new File("/media/exdisk/Sem06/BA/Neuromorpho/Metadaten");
        NeuronMetadataMapper mapper = new NeuronMetadataMapper();
        Map<String, NeuronMetadataRImpl> neuronMetadataRImpl = mapper.mapFromDirectory(metadataPath);
        Float somaSurface;
        for (String neuronName : neuronMetadataRImpl.keySet()) {
//            System.out.println(neuronName);
            somaSurface = neuronMetadataRImpl.get(neuronName).getCorrectedXy();
            if (somaSurface != null) {
                System.out.println(somaSurface.toString());
            }
        }
        System.out.println(neuronMetadataRImpl.size());
